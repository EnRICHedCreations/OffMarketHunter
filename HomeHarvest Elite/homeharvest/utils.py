...
def validate_filters(
    ...
    hoa_fee_min: int | None = None,
    hoa_fee_max: int | None = None,
    stories_min: int | None = None,
    stories_max: int | None = None,
    garage_spaces_min: int | None = None,
    garage_spaces_max: int | None = None,
) -> None:
    ...
    ranges = [
        ...
        ("hoa_fee", hoa_fee_min, hoa_fee_max),
        ("stories", stories_min, stories_max),
        ("garage_spaces", garage_spaces_min, garage_spaces_max),
    ]
...
def validate_sort(sort_by: str | list[str] | None, sort_direction: str | list[str] | None = "desc") -> None:
    """Validate sort parameters."""
    valid_sort_fields = [
        "list_date", "sold_date", "pending_date", "last_sold_date", "last_update_date",
        "list_price", "sold_price", "price_per_sqft", "sqft", "lot_sqft",
        "beds", "full_baths", "baths", "year_built", "days_on_mls",
        "hoa_fee", "stories", "parking_garage", "assessed_value", "estimated_value",
        "property_age", "value_per_sqft", "price_discount", "lot_ratio"
    ]
    ...
    if isinstance(sort_by, list):
        for field in sort_by:
            if field and field not in valid_sort_fields:
                raise ValueError(
                    f"Invalid sort_by value: '{field}'. "
    ...
    # Handle list of directions
    if isinstance(sort_direction, list):
        for direction in sort_direction:
            if direction and direction not in valid_directions:
                raise ValueError(
                    f"Invalid sort_direction value: '{direction}'. "
                    f"Valid options: {', '.join(valid_directions)}"
                )
    elif sort_direction and sort_direction not in valid_directions:
        raise ValueError(
...
def validate_tag_filters(
    ...
    tag_match_type: str,
    tag_exclude: list[str] | None
) -> None:
    """Validate tag filtering parameters."""
    valid_match_types = ["any", "all", "exact"]

    if tag_match_type not in valid_match_types:
        raise ValueError(
            f"Invalid tag_match_type value: '{tag_match_type}'. "
            f"Valid options: {', '.join(valid_match_types)}"
        )
    ...
    if tag_filters is not None:
        ...
        if not tag_filters:
            raise ValueError("tag_filters cannot be an empty list")
        if not all(isinstance(tag, str) for tag in tag_filters):
            raise ValueError("All elements in tag_filters must be strings")

    if tag_exclude is not None:
        if not isinstance(tag_exclude, list):
            raise ValueError("tag_exclude must be a list of strings")
        if not tag_exclude:
            raise ValueError("tag_exclude cannot be an empty list")
        if not all(isinstance(tag, str) for tag in tag_exclude):
            raise ValueError("All elements in tag_exclude must be strings")
