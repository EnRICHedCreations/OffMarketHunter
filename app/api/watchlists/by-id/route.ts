...
// GET /api/watchlists/by-id?id=X - Get single watchlist
export async function GET(request: Request) {
  try {
    ...
    console.log(`[GET /api/watchlists/by-id] User ID: ${session.user.id}, Watchlist ID: ${id}`);
    ...
    if (checkExists.rows.length === 0) {
      console.log(`[GET /api/watchlists/by-id] Watchlist does not exist`);
      return NextResponse.json({ error: 'Watchlist not found' }, { status: 404 });
    }

    console.log(`[GET /api/watchlists/by-id] Watchlist exists with user_id: ${checkExists.rows[0].user_id}`);
    ...
    if (result.rows.length === 0) {
      console.log(`[GET /api/watchlists/by-id] User ${session.user.id} does not own this watchlist (owner: ${checkExists.rows[0].user_id})`);
      return NextResponse.json({ error: 'Watchlist not found' }, { status: 404 });
    }

    console.log(`[GET /api/watchlists/by-id] Successfully fetched watchlist`);
...
// PUT /api/watchlists/by-id?id=X - Update watchlist
...
// DELETE /api/watchlists/by-id?id=X - Delete watchlist
